...
import {Client, Guild, GuildMember, GuildTextBasedChannel, Message, MessageEmbed, User} from 'discord.js';
...
export async function sendLogMessage(logger: ILogger, url: string, channel: string, member: GuildMember, client: Client, deleted: boolean, emojis: string[]) {
    ...
    const addToMessage = ref && ref.lastReport + 600000 > Date.now() && ref.messageRefs[0]?.channel.toString() === channel;
    if (ref && !addToMessage) {
    ...
    if (addToMessage) {
        ...
        for (const msg of ref.messageRefs) {
            msg.edit({ embeds: [embed]}).catch(err => logger.error(err));
        }
        ref.count = count;
    } else {
        ...
        client.reportedMessages.push({ id: member.id, count, emojis: previousEmojis, messageRefs, lastReport: Date.now()})
    }
}
